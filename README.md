...
Auparavant, mon reseau local était sur le réseaux 192.168.254.0/24. Hors
maintenant pour concervoir le réseau de mon homelab, je désire utiliser les
capacité des VLAN.

Afin de respecter la nomenclature de l'ancien réseaux, j'ai découpé les réseaux
avec un masque de sous réseaux de **`/20`**, ce qui me permet d'avoir :

- 16 grande catégorie VLAN
- 16 VLAN par catégorie

Dans chaque réseaux de `/24`, dans la mesure du possible je catégoriserais par
type de dévice avec un masque de **`/28`** ce qui fera une capacité de 16
devices

![Network diagram](./docs/network.svg)

#### VLAN zone

| VLAN zone                     | net              |
| ----------------------------- | ---------------- |
| Private (LAN, ADM)            | 192.168.240.0/20 |
|                               | 192.168.224.0/20 |
|                               | 192.168.208.0/20 |
|                               | 192.168.192.0/20 |
|                               | 192.168.176.0/20 |
...
|                               | 192.168.64.0/20  |
|                               | 192.168.48.0/20  |
| Demilitarized zone (DMZ, CTTV | 192.168.32.0/20  |
|                               | 192.168.16.0/20  |
| War zone (WAR)                | 192.168.0.0/20   |

#### VLAN allocation

|     VLAN Zone      | VLAN ID | VLAN Network |     address      |    description     |
| :----------------: | :-----: | :----------: | :--------------: | :----------------: |
|      Private       |   254   |     lan      | 192.168.254.0/24 | Local area network |
|      Private       |   240   |     adm      | 192.168.240.0/24 |   Admin resource   |
| Demilitarized zone |   32    |     dmz      | 192.168.32.0/24  |                    |
|      War zone      |    0    | internet box |  192.168.0.0/24  |                    |

#### Categorized device

| Prefix device | description      | device network   | used in vlan    | devices                               |
| ------------- | ---------------- | ---------------- | --------------- | ------------------------------------- |
| mikrotik      | router           | 192.168.x.240/28 | adm,lan,dmz,war | mikrotik254, mikrotik253, mikrotik252 |
| hype          | Incus hypervisor | 192.168.x.16/28  | adm,lan,dmz     | hype16                                |

#### Network configuration

[ [mikrotik254](./docs/hosts/mikrotik254)
...
The variable `PASSWORD_STORE_DIR` is configured to
`/home/badele/ghq/github.com/badele/pass`, you can clone your passwordstore with
the `ghq get git@github.com:badele/pass.git`
...
- `age-keygen | pass insert -m nix-homelab/age/your_username`
- `pass show nix-homelab/age/your_username | grep AGE-SECRET-KEY >> ~/.config/sops/age/keys.txt`
...
- From your desktop
  - `nix develop`
  - **Optional**
    - `just nixos-init-host <HOSTNAME>`
  - `just iso-build`
    - Write the ISO image on USB key or Zalman CD-ROM emulator
  - Boot
    - Get ip with `ip a`
  - `just nixos-install <HOSTNAME> <TARGETIP>`
...
```bash
just nixos-remote-update <hostname> <IP>
```
...
Available recipes:
    ...
    nixos-remote-update hostname targetip options=""    Install on remote host
