...
in
{
  ...
  options.homelab.features.${appName} = mkFeatureOptions {
    extraOptions = {
      ...
      appSubDomain = mkOption {
        type = str;
        default = "miniflux";
        description = "Subdomain for ${appName} service";
      };
    ...
    (mkIf cfg.enable {
      ...
      services.miniflux = {
        ...
        config = {
          ...
          OAUTH2_CLIENT_ID = "${cfg.appSubDomain}-${appName}";
          OAUTH2_REDIRECT_URL = "${exposedURL}/oauth2/oidc/callback";
          OAUTH2_OIDC_DISCOVERY_ENDPOINT = "https://${cfg.authDomain}/application/o/${cfg.appSubDomain}-${appName}/";
