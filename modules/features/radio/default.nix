  ...
  config =
    ...
    mkMerge [
      ...
      (mkIf cfg.enable {
        ...
        # Enable radio in TLS mode with caddy reverse proxy if openFirewall is enabled
          ...
          "${cfg.serviceDomain}" = {
            ...
            extraConfig = ''
              ...
              header {
                ...
                Content-Security-Policy "default-src 'self'; font-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' https://unpkg.com; img-src 'self' data:; media-src 'self' blob: http: https:; connect-src 'self' http: https: ws: wss:;"
